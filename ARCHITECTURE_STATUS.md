...
Runtime storage is portable and rooted beside the executable (or beside the source tree in development): `app_data/`, `camera_support/` and `exported_styles/`. Camera-preparation artefacts are never automatically pruned or overwritten; microsecond timestamp plus style name gives each attempt a distinct folder. Legacy AppData settings are copied forward without deleting the source.
...
The validated Manual Loader 2.4 workflow is now coordinated by the editor UI but remains isolated in `camera_install/`. EOS RP installation is not equivalent to PF3 generation:

`PF3 → Canon compiler → 16744 legacy representation → exact 8192-byte Block1 → known EOS RP carrier → hook genuine EOS Utility 0x01000203 transaction`

The compiler byte-for-byte self-test is mandatory: both known-good legacy blocks must exactly match the validated 8192-byte oracle. If it fails, installation must abort rather than attempt a best-effort payload write. For the target PF3, the documented and physically validated V37 recipe extracts exact legacy Block1; target Block2 may legitimately differ and remains the carrier's unchanged native Block2. EOS Utility remains transaction owner and no GUI auto-click path is permitted.
...
The physically demonstrated carrier result is limited to: compiled target Block1 plus the known-good native EOS RP carrier. It does not prove that Block2 is universally irrelevant, and it does not prove that a standalone arbitrary PF3 can be registered without the loader.

`camera_install/rp_assets.py` requires the five external research fixtures to match their validated sizes and SHA-256 hashes. No carrier, descriptor, captured payload, Canon PF3 or compiler reference is distributed. `camera_install/rp_payload.py` implements the fail-closed legacy/Block1/carrier checks. `camera_install/eos_hook.py` manages Frida and EOS Utility. `camera_install/ui.py` captures immutable editor state and coordinates the user workflow.

Send to Camera is physically validated only for EOS RP. RAW compatibility with another Canon body does not imply installation compatibility. The UI requires explicit EOS RP confirmation and advises closing/reopening EOS Utility between successive registrations until its state-related reliability is better understood.
